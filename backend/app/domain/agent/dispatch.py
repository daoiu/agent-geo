...
②a Agent 路径统一到 LangGraph(2026-07-17 计划):LangGraph 成为唯一
agent 执行路径。react_loop 后备路径已删除(详见 plan Task 10)。
...
async def _run_langgraph_turn(session_id: str, message: str) -> AsyncIterator[bytes]:
    ...
    ②a 唯一 agent 执行路径(react_loop 后备已删除,plan Task 10)。
    """
