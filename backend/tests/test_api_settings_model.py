...
def test_reset_clears_overrides_and_falls_back_to_env(client, monkeypatch):
    # 先在不带 JSON 覆盖的情况下记下 .env 的 baseline
    from app.core.config import get_settings
    baseline = get_settings().deepseek_model
    client.patch("/api/settings/models", json={
        "providers": [{"name": "deepseek", "model": "deepseek-v3"}]
    })
    assert get_settings().deepseek_model == "deepseek-v3"
    ...
    assert r.status_code == 200
    # fallback 到 .env baseline（具体值由 host 的 .env 决定）
    assert get_settings().deepseek_model == baseline
    assert r.json()["source"] == "env"
