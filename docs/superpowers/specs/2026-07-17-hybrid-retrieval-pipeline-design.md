...
- **语义缓存**:真 Redis(`redis` / redis-py + `docker-compose` 加 redis 服务);按 query embedding 余弦相似度命中;**扫描有界**——用 Redis ZSET(score=时间戳)维护 LRU 最近窗口,每次只对最近 `semantic_cache_max_scan`(默认 1000)条候选算余弦,避免全量 O(N) 扫描;单测 mock,集成测试 gated(`REDIS_URL` 不可达自动 skip)。V2 演进路径:RediSearch / RedisVL 的 HNSW 向量索引做毫秒级 ANN(见 §非目标)。
...
- **分词与领域词典**:BM25/关键词召回依赖分词质量,`jieba` 默认词典会把 `LangChain`/`GPT-4o`/`Claude-3.5` 等专有名词切碎,直接损害召回。故引入 **GEO 领域用户词典**(`jieba.load_userdict()`),启动时加载,作用于**全代码库所有 jieba 调用**(现有 `search_chunks_by_keyword` / `search_chunks_all_keywords` / 跨库检索同享红利);词典由**现有品牌表 + 精选技术术语清单**播种。`jieba_fast` 仅作可选加速项(C 扩展,解决速度非切碎,且无外网沙箱可能编译失败,默认不启用)。
- **顺序**:本子系统(①)在 ④ 之后做。
...
| `backend/app/services/retrieval/tokenizer.py` | 领域分词:启动加载 GEO 用户词典(品牌名+模型名+技术术语),统一 jieba 切词入口 | 新增 |
| `backend/app/services/retrieval/bm25_search.py` | 用 `rank_bm25.BM25Okapi` 打分,经 `tokenizer` 分词;对 KB chunk 语料排序取 top-k | 新增 |
...
| `backend/app/services/retrieval/semantic_cache.py` | `SemanticCache`(Redis 后端):ZSET 维护 LRU 窗口 + 有界余弦扫描(≤ `max_scan`)+ TTL;无 Redis 时 no-op | 新增 |
...
- **语义缓存命中(有界扫描)**:进来的 query 先 embed(复用 `EmbeddingService`)。缓存条目存两部分——(a) 每条 `cache:{id}` 存 query 文本/embedding/结果 JSON,带 TTL;(b) 一个 ZSET `cache:recent`(member=id,score=时间戳)维护最近顺序。查询时 `ZREVRANGE cache:recent 0 max_scan-1` 取最近 ≤N 条 id,只对这批算余弦,≥阈值即命中。**扫描量恒定上界**(默认 1000),延迟不随缓存总量线性增长。这是简历「延迟 <50ms / 降 90%」的来源(命中路径跳过 LLM 改写 + 重排)。
- **分词(领域词典)**:`tokenizer.py` 启动时 `jieba.load_userdict(geo_terms.txt)`,词典含品牌名(从 brands 表播种)+ AI 模型名 + 技术术语,保证 `GPT-4o`/`LangChain` 等不被切碎。所有 jieba 调用统一走此入口。
- **BM25**:`bm25_search` 载入 KB chunk,经 `tokenizer` 分词构 `BM25Okapi`,对 query 分词 `get_scores` 排序取 top-k(与向量召回并列作为一路)。
...
- **单测**(离线可跑):`tokenizer` 领域词典切词(专有名词不被切碎);`bm25_search` 打分正确性;`query_rewrite` mock LLM + 无 key 降级;`reranker` mock / 恒等降级;`semantic_cache` mock redis + 语义命中 / 未命中 / TTL / **有界扫描(超 max_scan 只取最近 N)**;`hybrid_search` 编排(mock 各依赖,验证降级链)。
...
3. `pyproject.toml` 加 `rank-bm25`、`redis` 依赖;`docker-compose.yml` 加 redis 服务;`.env.example` 加新配置项;`backend/data/geo_terms.txt` 领域词典(品牌表播种 + 精选术语)。
...
- 评测框架本身(④ 已覆盖)。
- ② Agent 编排反思;③ OCR / VLM。
- **RediSearch / RedisVL 的 HNSW 向量索引**:本期语义缓存用「ZSET LRU 窗口 + 有界余弦扫描(≤ max_scan)」,扫描量恒定上界、当前规模足够。列为 **V2 演进路径**——当缓存条目规模增长到有界扫描不够时,引入 RediSearch/RedisVL HNSW 实现毫秒级 ANN 向量去重。接口预留,本期不实现。
